...
class UserController extends AdminController
{
    public function index($request, $response, $args)
    {
        $table_config['total_column'] = array('op' => '操作', 'id' => 'ID', 'user_name' => '用户名',
            ...
            'top_up' => '累计充值');
        $table_config['default_show_column'] = array('op', 'id', 'user_name', 'email', 'remark');
...
//        $user->ref_by = $request->getParam('ref_by');  //取消归属更新
    ...
    public function ajax($request, $response, $args)
    {
        ...
        if ($search) {
            ...
            $count_filtered = User::where(
                ...
                if(($this->AdminLevel !=1)) {
                    $count_filtered = $count_filtered->where($where)->count();
                }else{
                    $count_filtered = $count_filtered->count();
                }
            ...
            if(($this->AdminLevel !=1)) {
                $count_filtered = User::where($where)->count();
            }else{
        ...
        foreach ($users as $user) {
            ...
            $tempdata['op'] = ($this->AdminLevel!=1) ? '' : '<a class="btn btn-brand" href="/admin/user/' . $user->id . '/edit">编辑</a>
                    <a class="btn btn-brand-accent" id="delete" href="javascript:void(0);" onClick="delete_modal_show(\'' . $user->id . '\')">删除</a>';
            $tempdata['op'] .= '<a class="btn btn-brand" id="changetouser" href="javascript:void(0);" onClick="changetouser_modal_show(\'' . $user->id . '\')">切换为该用户</a>';
        ...
        $info = [
            ...
            'recordsTotal' => $count_filtered, //User::count(),
