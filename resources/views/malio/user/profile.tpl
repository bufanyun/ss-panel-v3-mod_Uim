...
{if $config['use_new_telegram_bot'] == true}
      <div class="modal-body">
        <p><p>复制下方密钥私聊发给 Telegram 机器人 <a href="https://t.me/{$telegram_bot}">@{$telegram_bot}</a> 以绑定 Telegram
        <!--<button class="btn btn-icon icon-left btn-primary copy-text" type="button" data-clipboard-text="{$bind_token}">点击拷贝</button>--></p>
        <!--<script>
           $(function () {
        ...
        <h2><code id="code_number" style="color: #6777ef">{$bind_token}</code></h2>
        </div>
        <p>手机电脑平板等如已安装 Telegram 可点击下方按钮一键绑定 </p>
        <div class="text-center">
        <a button class="btn btn-icon icon-left btn-primary" href="https://t.me/{$telegram_bot}?start={$bind_token}">一键绑定至账户</a>
        </div>
        </div>
        {else}
        <div class="modal-body">
        ...
        {/if}      <div class="modal-footer bg-whitesmoke br">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">{$i18n->get('cancel')}</button>
      </div>
    </div>
  </div>
