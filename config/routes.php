...
if($agent == false) {
    //返回默认域名配置
    $baseUrl = str_replace(array('http://', 'https://'), array('', ''), $_ENV['baseUrl']);
    $agent = AgentConfig::domain($baseUrl);
}
$_ENV['agent']   = $agent;
$_ENV['appName'] = $agent->panel['name'];
$_ENV['baseUrl'] = 'http://' . $agent->domain;
$_ENV['subUrl'] = $_ENV['baseUrl'].'/link/';
$_ENV['smtp_name'] = (string)$_ENV['appName'];

$Malio_Config['enable_chatra'] = ($agent->panel['enable_chatra'] == '1') ? true : false;    // 是否启用 Chatra 在线客服系统 https://chatra.io
$Malio_Config['chatra_id'] = (isset($agent->panel['chatra_id'])) ? $agent->panel['chatra_id'] : '';    //kEPtGmPvGWrWTsF3C Chatra 的 ChatraID，可以在 Chatra 提供的网站代码里找到

$I18N['appName'] = array(
    'en' => 'YJ2C',
    'zh-cn' => $agent->panel['name']
);
...
$I18N['login_slogan'] = array(
    ...
    'zh-cn' => '即便探索的道路上步满荆棘<br>' . $I18N['appName']['zh-cn'] . '也能带你走出繁华的风景！'
);
$I18N['index-slogan'] = array(
    'en' => 'We Dedicate to Providing the Finest Network Proxy Service',
    'zh-cn' => '欢迎来到'. $I18N['appName']['zh-cn']
);
...
$app->get('/aup', App\Controllers\HomeController::class . ':aup');
...
$app->group('/user', function () {
    ...
    //开通代理km
    $this->get('/agent', App\Controllers\UserController::class . ':agent');
    $this->post('/coupon_check_agent', App\Controllers\UserController::class . ':CouponCheckAgent');
    $this->post('/agent_buy', App\Controllers\UserController::class . ':agentBuy');
...
$app->group('/admin', function () {
    ...
    //卡密管理
    $this->get('/km', App\Controllers\Admin\KmController::class . ':index');
    $this->get('/km/{id}/edit', App\Controllers\Admin\KmController::class . ':edit');
    $this->post('/km/changetouser', App\Controllers\Admin\KmController::class . ':changetouser');
    $this->post('/km/ajax', App\Controllers\Admin\KmController::class . ':ajax');
    $this->post('/km/create', App\Controllers\Admin\KmController::class . ':create');
    $this->get('/km/log', App\Controllers\Admin\KmController::class . ':log');
    $this->post('/km/log_ajax', App\Controllers\Admin\KmController::class . ':log_ajax');
    $this->get('/km/{batch}/txt', App\Controllers\Admin\KmController::class . ':log_txt');

    //帮助中心
    $this->get('/help', App\Controllers\Admin\ConfigController::class . ':help');

    //站点配置
    $this->get('/config', App\Controllers\Admin\ConfigController::class . ':index');
    $this->get('/config/edit_email', App\Controllers\Admin\ConfigController::class . ':edit_email'); //修改邮箱
    $this->get('/config/dupdate', App\Controllers\Admin\ConfigController::class . ':dupdate');  //域名信息
    $this->post('/config/cupdate', App\Controllers\Admin\ConfigController::class . ':cupdate');  //上传提现收款码
    $this->post('/config/update_cash', App\Controllers\Admin\ConfigController::class . ':update_cash');  //修改提现信息
    $this->post('/config/update_chatra', App\Controllers\Admin\ConfigController::class . ':update_chatra');  //在线客服


    /* 代理管理 */
    //代理列表
    $this->get('/agent/lists', App\Controllers\Admin\AgentController::class . ':lists');
    $this->post('/agent/lists_ajax', App\Controllers\Admin\AgentController::class . ':lists_ajax');

    //修改
    $this->get('/agent/{id}/lists_edit', App\Controllers\Admin\AgentController::class . ':lists_edit');
    $this->post('/agent/lists_edit_post', App\Controllers\Admin\AgentController::class . ':lists_edit_post');

    //添加
    $this->get('/agent/lists_create', App\Controllers\Admin\AgentController::class . ':lists_create');
    $this->post('/agent/lists_add', App\Controllers\Admin\AgentController::class . ':lists_add');
    $this->delete('/agent/detect',  App\Controllers\Admin\AgentController::class . ':delete');

    //等级修改
    $this->get('/agent/type', App\Controllers\Admin\AgentController::class . ':type');
    $this->post('/agent/type_ajax', App\Controllers\Admin\AgentController::class . ':type_ajax');
    $this->get('/agent/{id}/type_edit', App\Controllers\Admin\AgentController::class . ':type_edit');
    $this->post('/agent/type_edit_post', App\Controllers\Admin\AgentController::class . ':type_edit_post');

    //收益记录
    $this->get('/agent/income', App\Controllers\Admin\AgentController::class . ':income');
    $this->post('/agent/income_ajax', App\Controllers\Admin\AgentController::class . ':income_ajax');

    //提现记录
    $this->get('/agent/cash', App\Controllers\Admin\AgentController::class . ':cash');
    $this->post('/agent/cash_ajax', App\Controllers\Admin\AgentController::class . ':cash_ajax');
    $this->post('/agent/cash_post', App\Controllers\Admin\AgentController::class . ':cash_post');

    //提现管理
    $this->get('/agent/cash_log', App\Controllers\Admin\AgentController::class . ':cash_log');
    $this->post('/agent/cash_log_ajax', App\Controllers\Admin\AgentController::class . ':cash_log_ajax');
    $this->get('/agent/cash_log/{id}/{status}', App\Controllers\Admin\AgentController::class . ':cash_log_edit');

    //余额明细
    $this->get('/detailed', App\Controllers\Admin\DetailedController::class . ':index');
    $this->post('/detailed/ajax', App\Controllers\Admin\DetailedController::class . ':ajax');
    ...
    $this->put('/shop/{id}/edit_post', App\Controllers\Admin\ShopController::class . ':update');//下级代理更新路由
